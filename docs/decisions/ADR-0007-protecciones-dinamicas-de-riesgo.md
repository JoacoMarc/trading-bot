...
- Cada transición genera un evento `protection_triggered` / `protection_cleared` en el `TradeStore`, con `reason` y detalle, visible en `REPORT.md` y, desde la Fase 7, en `status.json`. Los `entry_rejected` se registran una vez por par mientras el mismo motivo se repita en velas consecutivas con señal (las `stats` cuentan todos); con `entry_mode=state` un halt largo no llena el store y con `cross` cada señal rechazada queda registrada.
