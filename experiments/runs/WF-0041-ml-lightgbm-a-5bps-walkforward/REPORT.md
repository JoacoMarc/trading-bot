# WF-0041 — donchian 4h walk-forward IS 24 m / OOS 6 m (fijo)
...
- Reproducibilidad: git af1b65f, params c8d4a6af5b, datos e61213fad5d5, 56.7 s en MacBook-Air-4.local
...
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | 2019-08-01 -> 2021-08-01 | 2021-08-01 -> 2022-02-01 | base | -0.01 % | -0.02 | 0.38 % | 3 | 0.97 | 0 | 6 |
...
|---|---|---|---|---|
| Retorno total | -0.01 % | +175.83 % | +214.53 % | +174.95 % |
| CAGR | -0.00 % | +28.87 % | +33.17 % | +28.77 % |
| Sharpe (diario) | -0.01 | 0.76 | 1.07 | 0.72 |
| Sortino (diario) | -0.01 | 1.12 | 1.74 | 1.03 |
| Calmar | -0.01 | 0.37 | 1.23 | 0.35 |
| Max drawdown / mayor tramo bajo agua | +0.38 % / 1361 d | +77.11 % / 852 d | +26.93 % / 438 d | +81.53 % / 1108 d |
| Profit factor | 0.97 | — | — | — |
| Win rate | +33.33 % | — | — | — |
| Expectancy por trade | -0.19 | — | — | — |
| Trades / duración media | 3 / 104.0 h | 0 / — h | 0 / — h | 0 / — h |
| Exposición | +0.54 % | +100.00 % | +100.00 % | +100.00 % |
| Fees pagados / shortfall medio | 2.39 / 5.1 bps | 0.00 / — bps | 0.00 / — bps | 0.00 / — bps |


Rango OOS 2021-08-01 -> 2025-08-01. Equity encadenada 10,000.00 -> 9,999.08 USDT.

Muestra completa con parámetros fijos (2019-08-01 -> 2025-08-01): retorno +0.98 %, Sharpe 0.21, max DD +1.03 %, 15 trades.

## Gate 1 — backtest -> paper: incompleto (16 criterios sin evaluar)

| Criterio | Umbral | Valor | Resultado | Nota |
|---|---|---|---|---|
| Sharpe OOS | >= 0.8 y >= B&H BTC (0.76) | -0.01 | n/a | ; evidencia ML con cobertura incompleta |
| Profit factor OOS | >= 1.3 | 0.97 | n/a | ; evidencia ML con cobertura incompleta |
| Max DD OOS | <= 25 % y <= 50 % del DD B&H (77.11 %) | 0.38 % | n/a | ; evidencia ML con cobertura incompleta |
...
| Régimen: retorno 2021 | > 0 | +0.98 % | n/a | muestra completa, parámetros fijos; evidencia ML con cobertura incompleta |
...
| Régimen: max DD intra-año | <= 25 % | 1.03 % (2021) | n/a | muestra completa, parámetros fijos; evidencia ML con cobertura incompleta |
...
5,000 corridas sobre 3 trades, semilla 42: max DD p50 +0.19 %, p95 +0.38 %, p99 +0.57 %; retorno p05 -0.38 %, p50 -0.01 %.

Informativo (ADR-0010), bootstrap por bloques de 20 días de los 1460 retornos diarios OOS: max DD p50 +0.32 %, p95 +0.57 %, p99 +0.67 %; retorno p05 -0.32 %, p50 +0.00 %.

## Meseta ±20 %

24 variantes, pasan +58.33 % (PF > 1.1 y retorno > 0). Informativo: +58.33 % de las variantes tiene Sharpe >= 0.5 x el base (0.21). Las 8 peores:

| Cambios | Retorno | PF | Sharpe | Trades | Pasa |
|---|---|---|---|---|---|
| atr_mult=2.4, prediction_threshold=0.55 | -0.50 % | 0.77 | -0.11 | 20 | no |
